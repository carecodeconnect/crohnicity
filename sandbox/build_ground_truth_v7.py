...
df["reasons_for_biologic_not_taken"] = df["reasons_for_biologic_not_taken"].astype(
    "object"
)
df.loc[taken, "reasons_for_biologic_not_taken"] = "BIOLOGIC_TAKEN"
print(
    f"set BIOLOGIC_TAKEN for {int(taken.sum())} biologic-taken cases: {list(df.loc[taken, 'patient_id'])}"
)
