...
> **SSOT rule.** The *Actual* cells **link to** the rendered figures in `SOLUTION.md` and the
> persisted predictions in [`data/out/json/`](../data/out/json) rather than restating literals
> that drift between runs — the SOLUTION write-up is itself Quarto-rendered from `data/out/`, so each link
...
- **Demographic skew in pathway/outcome.** `Demographics{gender, age}` is extracted to enable a
  later "do pathways/outcomes differ by demographic" cut (SOLUTION → *Pydantic Schema Design*), but
  that cut was deferred. **Untested.**
- **Churn / truncation rate.** The hypothesis scan flagged "some transcripts cut off mid-journey."
  The model's `churn` flag (see [SOLUTION → Churn](SOLUTION.md#churn--definition--handling)) is
...
*Actual* column and the open list above are kept honest by linking to the SSOT (`SOLUTION.md` /
