# Crohnicity — solution & evaluation


<!-- Source of truth: edit docs/SOLUTION.qmd, then `uv run quarto render docs/SOLUTION.qmd` to regenerate
     SOLUTION.md (gfm) + SOLUTION.html (local verification). The analysis chunks read only the
     persisted predictions in data/out/ — no Gemini API calls. -->

***Crohnicity*** = **Crohn’s** + **chronicity** — the disease and its
time dimension. Crohn’s is a chronic, lifelong condition, and the
health-tech AI companion records each patient’s treatment journey
...
- **Reason enums — aligned to the spec, departing only where the
  ...
  [`SCHEMA.md`](SCHEMA.md) v0.7): **(1)** we keep `INSURANCE_PROBLEMS`
  rather than the spec’s bare *“access”* — every real barrier in the
  data is payer-gating (step-therapy / prior-auth / tier), not drug
  *availability*, and `ACCESS` had **zero** transcript support (covered
  by `COST` + `INSURANCE_PROBLEMS`); **(2)**
...
> ↔ `uv run` relation are drawn in [SYSTEM_DESIGN.md](SYSTEM_DESIGN.md).
...
- **Determinism & reproducibility.** Three levers, in decreasing order
  ...
  `data/out/`, and the whole analysis (this document) reads *only* those
...
> (present in 27/50) — the point a biologic-prescribing specialist is
...
Crucially, **47/50 journeys are cyclic** — the patient loops back
...
linear step number alone understates the journey for the **94%** of
...
[referral_pathways.md](referral_pathways.md), and the **interactive**
pyvis graphs in [`data/out/html/`](../data/out/html/) (open
`index.html`) — these render **live in a browser from a local clone**;
GitHub only shows them as source. As a teaser, here is patient **P046**
...
| discussed but not taken (negation) |       12 |
| churned / truncated                |        2 |

The model flagged `churn = true` for only **2/50** patient(s). Manual
...
the patient’s interaction with the health-tech AI companion app —
...
- **Dagster orchestration.** The whole pipeline is an asset graph
  ...
  run standalone from the CLI ([SYSTEM_DESIGN.md](SYSTEM_DESIGN.md)).
...
- **Reproducible reporting + an API docsite.** This write-up is
...
- **A telemetry feature for optimisation.** Per-call usage is currently
  ...
  [TELEMETRY.md](TELEMETRY.md)); with more time I’d surface it as a
  first-class artifact (a structured per-run metrics file + plots) to
  track cost/latency/cache-hit rate and tune the pipeline at scale.
...
- **Lift test coverage on the orchestration + LLM-call paths.** The gate
  ...
  in this write-up) at 98%. The gaps fall **exactly on the
  side-effecting I/O boundary** — the code you can’t exercise without
  mocking an external service: the live `litellm` call in `extract.py`
  (36%) and the `main.py` / `pipeline.py` entry points (0% — the fire
  CLI + the Dagster assets, which shell out to Gemini, `quarto` and
  `mkdocs`). So the untested code is the **plumbing, not the analysis**.
  With more time I’d **mock the `litellm` call** to exercise the retry /
...
- After clarifying the project specification and system design and how I
  ...
  the CC chat with `/export` as I went, to review the workflow. CC
...
- Most of the docs in `docs/` were generated with CC, so there is a
  ...
  `SOLUTION.qmd` is programmatic to ensure all the data values are
...
orchestrates the whole pipeline end-to-end — extract → (EDA/SOLUTION
...
> (data + logs + SOLUTION) is your separate `git` step.
...
The post-extraction EDA (this write-up’s tables/plots) is generated by
`uv run quarto render docs/SOLUTION.qmd`, which reads only `data/out/` —
...
``` bash
uv run quarto render docs/SOLUTION.qmd                  # regenerate SOLUTION.md (gfm) + SOLUTION.html from data/out/
```
...
> version-controlled. See [QUESTIONS.md](QUESTIONS.md) \#7.
...
workflow — are documented once in [DEV_SETUP.md](DEV_SETUP.md); this
write-up only links there to avoid duplication.
...
- [SYSTEM_DESIGN.md](SYSTEM_DESIGN.md) — architecture diagram: the
  Dagster asset graph + the library-first CLI/gate relation.
- [SCHEMA.md](SCHEMA.md) — the ground-truth annotation schema and the
  Pydantic-model rationale.
- [QUESTIONS.md](QUESTIONS.md) — outstanding questions for the CAIO
  (assumptions needing a domain decision).
- [HYPOTHESES.md](HYPOTHESES.md) — early hypotheses about the data and
  the disease journey.

*Process & operations*

- [DEV_SETUP.md](DEV_SETUP.md) — local dev setup: uv, the Jupyter
  kernel, Dagster (`dg`), the quality gate.
- [TODO.md](TODO.md) — deferred engineering tasks + the *Tomorrow —
  pre-interview* checklist (the clean `temperature=0` re-run).
- [TELEMETRY.md](TELEMETRY.md) — per-call API telemetry (tokens, cached
  tokens, cost, latency) and how to read it.
- [TO_REVIEW.md](TO_REVIEW.md) — the manual gold-set edge-case audit
  (the churn false-positive / false-negative findings).
- [DEBUGGING_ERROR_HANDLING.md](DEBUGGING_ERROR_HANDLING.md) — debugging
  notes + error-handling behaviour seen in development.

*Reference*

- [RESOURCES.md](RESOURCES.md) — external references (Crohn’s pathways,
  biologics).
- [TASK_INSTRUCTIONS.md](TASK_INSTRUCTIONS.md) — the original
  health-tech take-home brief.
...
│   ├── post_extraction_eda.py        #   pure analysis fns over data/out (power the write-up + tests)
...
└── docs/SOLUTION.qmd → docs/SOLUTION.md   # this write-up (edit the .qmd, render to .md)
```
