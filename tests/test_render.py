...
Runs `quarto render docs/SOLUTION.qmd` and checks the GFM output is clean: no unresolved inline
...
def test_readme_renders_clean() -> None:
    """`quarto render docs/SOLUTION.qmd` succeeds and produces drift-free, CSS-free GFM."""
    if os.getenv("RUN_RENDER_TEST") != "1":
        pytest.skip("RUN_RENDER_TEST != 1 — quarto render smoke test skipped")
    result = subprocess.run(
        ["uv", "run", "quarto", "render", "docs/SOLUTION.qmd"],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
