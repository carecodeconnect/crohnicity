...
GT(q1).tab_header(title="Q1 — patients on a biologic")
```
...
GT(q1eval).tab_header(title="Q1 — biologic_taken: model vs gold")
```
...
GT(q2).tab_header(title="Q2 — reasons not on a biologic")
```
...
GT(q3).tab_header(title="Q3 — treatments tried before a biologic")
```
...
> **Caveat.** The literal "GP" node (`primary_care_contact`) appears in only 5/50 predicted
> pathways, so a strict GP→prescriber count isn't representative. We count steps from the journey
> **start** (or `primary_care_contact` where present) to `biologic_recommended` (present in 50/50)
> — the point a biologic-prescribing specialist is reached. Under-emission of the GP node is a
> prompt-fix candidate (see `docs/TODO.md`). Per-case journey graphs: `data/out/referral_pathway_P*.html`.
...
(
    ggplot(pd.DataFrame({"steps": steps.astype(int)}), aes("steps"))
    + geom_histogram(binwidth=1)
    + theme_grey()
    + labs(title="Q4 — steps from journey start to biologic recommendation", y="patients")
).save(PLOTS / "q4_steps.png", width=7, height=3.5, dpi=150, verbose=False)
q4 = (
    steps.describe()[["count", "min", "50%", "max", "mean"]]
    .round(1)
    .rename_axis("stat")
    .reset_index(name="steps")
)
GT(q4).tab_header(title="Q4 — steps to biologic recommendation")
```
...
A typical journey is **~`{python} int(steps.median())` steps** from start to a biologic recommendation (range `{python} int(steps.min())`–`{python} int(steps.max())`, n=`{python} int(steps.count())`).
...
GT(three_state).tab_header(title="Churn — three-way split")
```
